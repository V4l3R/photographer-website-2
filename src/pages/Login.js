...
const Login = () => {
  ...
  const { handleSetIsAdmin, handleSetAdminUsername, handleSetAccessToken } = useContext(AdminContext);
      ...
      <div className='flex items-center justify-center h-[100vh]'>
        <div className='flex flex-col gap-y-4 h-[60vh] w-[50vw] pt-20'>
          ...
          <div className='h-[19vh] w-full'>
            ...
            <div className=''>
              {/* <AdminConnected /> */}
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.0 }} style={{ fontWeight: "bold", color: "green", textAlign: "center" }} className={`w-full absolute left-0 -mt-6`} >{successMessage}</motion.div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.0 }} style={{ fontWeight: "bold", color: "red", textAlign: "center" }} className={`w-full absolute left-0 ${isLostPassword ? 'mt-2' : '-mt-6'}`} >{errorMessage}</motion.div>
              <div className='flex flex-col gap-y-4 md:px-[5vw] lg:px-[10vw] xxl:px-[15vw]'>
                {!isLostPassword &&
                  <>
                    <motion.input initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.1 }} type="text" name="identifiant" id="identifiant" placeholder='Adresse email' className='outline-none border-b border-b-primary h-[60px] bg-transparent font-secondary w-full pl-3 placeholder:text-[#757879]' onKeyUp={e => handleEnter(e, connect)} onChange={e => setUsername(e.target.value)} onBlur={(e) => validateEmail(e)} />
                    <motion.input initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.3 }} type="password" name="password" id="password" placeholder='Mot de passe' className='outline-none border-b border-b-primary h-[60px] bg-transparent font-secondary w-full pl-3 placeholder:text-[#757879]' onKeyUp={e => handleEnter(e, connect)} onChange={e => setPassword(e.target.value)} />
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.5 }} className='text-end cursor-pointer' onClick={toggleLostPassword}>Mot de passe oublié ?</motion.div>
                  </>
                }
                {isLostPassword &&
                  <>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.5 }} className='text-end cursor-pointer' onClick={toggleLostPassword}>Revenir à l'écran de connexion</motion.div>
                  </>
                }
              </div>
            </div>
          ...
          <div className='w-[10vw] self-center lg:ml-[20vw]'>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ transition: transition1, duration: 2.5 }} className=''>
              <SubmitButton />
            </motion.div>
          </div>
        </div>
