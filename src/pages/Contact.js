...
const Contact = () => {
  ...
  function SubmitButton() {
    if (name && validEmail && message) {
      return <button onClick={sendMail} className='btn mb-[30px] mt-8 mx-auto lg:mx-0 self-start'>Envoyer</button>
    } else {
      return <div style={{ opacity: 0.4 }} className='btn mb-[30px] mt-8 mx-auto lg:mx-0 self-start'>Envoyer</div>
    };
  };

  function MessageSended() {
    if (isMessageSended) {
      return <span style={{ fontWeight: "bold", color: "green", textAlign: "center" }} className='mb-[-10px]' >{sendedMessage}</span>
    }
  }
  ...
  function sendMail() {
    ...
    return http.post("/sendMail", formData, {
      headers: {
            ...
            <div onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler} className='lg:flex-1 lg:pt-32 px-4'>
              ...
              <div className='flex flex-col gap-y-4'>
                ...
                <div className='flex flex-col sm:flex-row gap-x-10'>
                  ...
                  <input type="text" name="email" id="userEmail" placeholder='Votre adresse email' className='outline-none border-b border-b-primary h-[60px] bg-transparent font-secondary w-full pl-3 placeholder:text-[#757879]' onChange={e => setEmail(e.target.value)} onBlur={(e) => validateEmail(e)}></input>
                </div>
